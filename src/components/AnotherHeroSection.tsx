...
const AnotherHeroSection = (props:SubHero) => {
  ...
  return (
    <div className={`${poppins.className} w-full h-[316px] relative flex flex-col items-center justify-center`}>
      ...
      <Image src={"/assets/logoletter.png"} alt='logo' className='z-10' height={20} width={40}/>
      <p className="text-[48px] font-medium text-black z-10">{props.mainhead}</p>
      <div className='flex gap-2'>
        <p className='font-medium text-[16px] z-10'>{props.homename}</p>
        <Image src={'/assets/greaterarrow.svg'} alt='left arrow' width={8} height={8} />
        <p className='font-light text-[16px] z-10'>{props.currentPage}</p>
      </div>
    </div>
  )
}
